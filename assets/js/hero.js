...
(function () {
  ...
  var STEPS = [
    { at: 0,    text: "ボタンを押しているあいだ、雨が降ります。" },
    { at: BASE + 0.05, text: "川の水位が、上がってきました。" },
    { at: 0.55, text: "水位計の赤い線に近づいています。" },
    { at: 0.85, text: "もうすぐ、堤防を越えます。" }
  ];
  var OVER_TEXT = "越えました。";
  ...
  function townY() { return lerp(TOWN_Y + 1, TOWN_Y - 34, town); }
  ...
  function rainDrops(dt) {
    ...
    ctx.strokeStyle = "rgba(235,244,255," + (0.35 + rain * 0.4) + ")";
    ctx.lineWidth = 1.3;
    ctx.beginPath();
    var wy = waterY();
  ...
  function step(now) {
    ...
    if (burst > 0) burst -= dt;
    var target = holding || burst > 0 ? 1 : 0;
    rain += (target - rain) * Math.min(1, dt * (target ? 3.5 : 1.8));

    if (!flooded) {
      level += rain * dt * 0.26;
      if (!holding && burst <= 0 && level < 1) level = Math.max(BASE, level - dt * 0.012);
    }
    if (level >= 1) {
      level = 1;
      if (!overflowed) {
  ...
  function up() {
    ...
    if (performance.now() - pressAt < 260) burst = 1.1;  // 短く押したら、一瞬の強い雨
  }
  ...
  reset.addEventListener("click", function () {
    level = BASE; town = 0; rain = 0; burst = 0;
